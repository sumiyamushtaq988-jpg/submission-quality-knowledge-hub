...
| ~~`{{NEWSLETTER_FORM_ACTION_URL}}`~~ | ✅ **No longer needed** — the contact & newsletter forms now work via Netlify Forms. (Submissions appear in Netlify → Forms. Swap to Mailchimp/Substack later if you want a true mailing list — see README §3.) | Done |
...
3. **Newsletter signups** now collect in **Netlify → Forms → newsletter** (the button works today). Optional: connect a real mailing-list provider (Mailchimp/Substack) later via README §3.
